...
public:
    sungjuk();
    sungjuk(int h, const char* name, int j1, int j2, int j3);
    void prin1(sungjuk *);
    void prin2(void);
};

sungjuk::sungjuk() {}
sungjuk::sungjuk(int h, const char* name, int j1, int j2, int j3) {
    hakbun = h;
    strncpy(irum, name, sizeof(irum));
    irum[sizeof(irum)-1] = '\0';  // null 종료 보장
    jumsu[0] = j1;
    jumsu[1] = j2;
    jumsu[2] = j3;
    tot = 0;
}
void sungjuk::prin1(sungjuk sung[]) {
    cout << "\n===== ===== ===== ===== ===== ===== ===== =====\n";
